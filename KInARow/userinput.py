...
def makeMove(state):
    ...
    utterance = "User puts a " + str(state[0]) + " at the position: ("+x+","+y+")"
    return [newState, utterance]

def makeGoodMove(state):
    global side
    startc = time.time()
    [bestScore, bestState] = minimax(state[0], state, 4, -1000000, 10000000, startc)
    global pState
    pState = bestState
    getUtterance = False
    utterance = ""
    while not getUtterance:
        if debug:
            ...
            if bestScore < 50:
                ...
                utterance = random.choice(uList)
        if len(oldUtterance) >= 12:
            oldUtterance = []
            oldUtterance.append(utterance)
            break
        if len(oldUtterance) > 0:
